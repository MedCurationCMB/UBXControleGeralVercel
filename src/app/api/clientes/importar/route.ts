...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Find "Cliente" column (case-insensitive)
    let clienteCol = 0
    sheet.getRow(1).eachCell((cell, colNum) => {
      if (String(cell.value ?? '').trim().toLowerCase() === 'cliente') clienteCol = colNum
    })
    if (!clienteCol) return NextResponse.json({ error: 'Coluna "Cliente" não encontrada' }, { status: 400 })

    const nomes: string[] = []
    sheet.eachRow((row, rowNum) => {
      if (rowNum === 1) return
      const val = String(row.getCell(clienteCol).value ?? '').trim()
    ...
    if (existentes.length > 0) {
      return NextResponse.json({
        error: `Os seguintes clientes já existem: ${existentes.join(', ')}`
      }, { status: 400 })
    }
    ...
    console.error('Importar clientes error:', err)
