  ...
  try {
    ...
    if (naoExistentes.length > 0) {
      return NextResponse.json({
        error: `Os seguintes clientes não existem no sistema: ${naoExistentes.join(', ')}. Cadastre-os primeiro.`
      }, { status: 400 })
    }
    ...
    for (const row of rows) {
      ...
      const { error: errUpd } = await supabaseServer.from('clientes').update({
    ...
    console.error('Importar clientes completo error:', err)
