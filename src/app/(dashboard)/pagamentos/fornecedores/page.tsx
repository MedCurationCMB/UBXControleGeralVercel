...
function validarCpfCnpj(v: string): { ok: boolean; digitos: string } {
...
function FornecedorModal({
  ...
  const handleSave = async () => {
    ...
    let payload: Partial<Fornecedor> = { ...form, nome: form.nome.trim() }

    // Validate + format CPF/CNPJ if filled
    if (form.cnpj_cpf) {
    ...
    const { error: err } = initial.id
      ? await supabase.from('fornecedores').update(payload).eq('id', initial.id)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    await supabase.from('fornecedores').delete().eq('id', initial.id)
    setDeleting(false)
    onSaved()
  }
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">
            {isEditing ? `Editar — ${initial.nome}` : 'Novo Fornecedor'}
          </h2>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <input className="input" value={form.rua_avenida ?? ''}
              onChange={e => set('rua_avenida', e.target.value)} />
          </div>
...
function ImportarModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const downloadTemplate = async (tipo: 'simples' | 'completo') => {
    const url = tipo === 'simples' ? '/api/fornecedores/template' : '/api/fornecedores/template-completo'
    const res = await fetch(url)
    if (!res.ok) return
    const blob = await res.blob()
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = tipo === 'simples' ? 'template_fornecedores.xlsx' : 'template_fornecedores_completo.xlsx'
    a.click()
  ...
  const handleImportSimples = async () => {
    ...
    const res = await fetch('/api/fornecedores/importar', { method: 'POST', body: fd })
    const data = await res.json()
    setSimplesLoading(false)
    if (!res.ok) { setSimplesError(data.error ?? 'Erro ao importar'); return }
    setSimplesSuccess(`${data.count} fornecedor(es) cadastrado(s) com sucesso!`)
    setSimplesFile(null)
    if (simplesRef.current) simplesRef.current.value = ''
    onSaved()
  }
  ...
  const handleImportCompleto = async () => {
    ...
    const res = await fetch('/api/fornecedores/importar-completo', { method: 'POST', body: fd })
    const data = await res.json()
    setCompletoLoading(false)
    if (!res.ok) { setCompletoError(data.error ?? 'Erro ao importar'); return }
    setCompletoSuccess(`${data.count} fornecedor(es) atualizado(s) com sucesso!`)
    setCompletoFile(null)
    if (completoRef.current) completoRef.current.value = ''
    onSaved()
  }

  const UploadZone = ({ file, onPick, inputRef, accept = '.xlsx' }: {
    file: File | null; onPick: (f: File) => void; inputRef: React.RefObject<HTMLInputElement | null>; accept?: string
  }) => (
    <div
    ...
    >
      {file ? (
        <div className="flex items-center justify-center gap-2">
          ...
          <button type="button" onClick={e => { e.stopPropagation(); onPick(null as unknown as File) }}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-xl w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">Importar Fornecedores em Lote</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
        </div>
        ...
        {tab === 'simples' && (
          <div className="space-y-4">
            <p className="text-sm text-slate-500">
              Importe uma lista de nomes para cadastro rápido. Os dados completos podem ser preenchidos depois editando cada fornecedor.
            </p>
            <p className="text-sm text-slate-600 bg-slate-50 border border-slate-200 rounded p-2">
              Coluna obrigatória: <strong>Fornecedor</strong>
            </p>
        ...
        {tab === 'completo' && (
          <div className="space-y-4">
            <p className="text-sm text-slate-500">
              Atualiza dados completos de fornecedores já cadastrados. O fornecedor deve existir pelo nome exato.
            </p>
...
export default function FornecedoresPage() {
  ...
  const [editTarget, setEditTarget] = useState<Partial<Fornecedor> & { id?: number } | null>(null)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const [{ data: forn }, { data: tipos }] = await Promise.all([
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter(f =>
        f.nome.toLowerCase().includes(q) || (f.cnpj_cpf ?? '').includes(q) ||
        (f.cidade ?? '').toLowerCase().includes(q)
      )
    }
    if (filtroStatus === 'completo') list = list.filter(isCadastroCompleto)
    if (filtroStatus === 'incompleto') list = list.filter(f => !isCadastroCompleto(f))
    return list
  }, [fornecedores, search, filtroStatus])

  const totalCompleto = useMemo(() => fornecedores.filter(isCadastroCompleto).length, [fornecedores])
  const incompletos = useMemo(() => fornecedores.filter(f => !isCadastroCompleto(f)), [fornecedores])
  const totalIncompleto = incompletos.length
  ...
  const handleCadastrarSimples = async (e: React.FormEvent) => {
    e.preventDefault()
    setNovoNomeMsg(null)
    if (!novoNome.trim()) { setNovoNomeMsg({ type: 'error', text: 'Digite o nome do fornecedor' }); return }
    setNovoNomeSaving(true)
    const { error } = await supabase.from('fornecedores').insert({ nome: novoNome.trim() })
    setNovoNomeSaving(false)
    if (error) { setNovoNomeMsg({ type: 'error', text: error.message }); return }
    setNovoNomeMsg({ type: 'success', text: `Fornecedor "${novoNome.trim()}" cadastrado com sucesso!` })
    setNovoNome('')
    load()
  ...
  const handleCompletarCadastro = () => {
    ...
    const fornecedor = incompletos.find(f => f.id === parseInt(completarId))
    if (fornecedor) setEditTarget({ ...fornecedor })
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="page-subtitle">Cadastro de fornecedores e parceiros</p>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap text-sm">
        <span className="text-slate-500">{fornecedores.length} fornecedor(es) total</span>
        <span className="flex items-center gap-1 text-green-700">
          <CheckCircle size={14} /> {totalCompleto} completo(s)
        </span>
        <span className="flex items-center gap-1 text-amber-600">
          <AlertCircle size={14} /> {totalIncompleto} incompleto(s)
        </span>
      </div>
      ...
      <div className="card">
        ...
        <form onSubmit={handleCadastrarSimples} className="flex gap-2">
          <input
            ...
            value={novoNome}
            onChange={e => { setNovoNome(e.target.value); setNovoNomeMsg(null) }}
          />
      ...
      <div className="card">
        ...
        <p className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
          {loading ? 'Carregando...' : `${filtered.length} de ${fornecedores.length} fornecedor(es)`}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="card text-center py-12 text-slate-400">Nenhum fornecedor encontrado.</div>
      ) : (
        <div className="card p-0 overflow-hidden">
          <div className="overflow-auto max-h-[480px]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map(f => {
                  const completo = isCadastroCompleto(f)
                  return (
                    <tr key={f.id} className="table-row">
                      <td className="table-cell font-medium text-slate-900">{f.nome}</td>
                      <td className="table-cell text-slate-500 font-mono text-xs">{f.cnpj_cpf || '-'}</td>
                      ...
                      <td className="table-cell text-xs">
                        {f.chave_pix ? (
                          <span>
                            <span className="text-slate-400 mr-1">[{tipoNome(f.tipo_chave)}]</span>
                            <span className="text-slate-700">{f.chave_pix}</span>
                          </span>
                        ) : '-'}
                      </td>
      ...
      {incompletos.length > 0 && (
        <div className="card space-y-3">
          <h2 className="text-base font-semibold text-slate-800">Completar Cadastro de Fornecedores</h2>
          <p className="text-sm text-slate-500">
            {incompletos.length} fornecedor(es) com dados incompletos. Selecione um para preencher as informações faltantes.
          </p>
          <div className="flex gap-2">
            <select
              className="input flex-1"
              value={completarId}
              onChange={e => setCompletarId(e.target.value)}
            >
              <option value="">Selecione o fornecedor para completar o cadastro...</option>
              {incompletos.map(f => (
