...
function RegistrarModal({
  ...
  // Load empresas
  useEffect(() => {
    supabase.from('categorias').select('empresa').then(({ data }) => {
      if (data) setEmpresas([...new Set(data.map(r => r.empresa as string))].sort())
    })
  }, [])

  // Load categorias when empresa changes
  useEffect(() => {
    if (!empresa) { setCategorias([]); setCategoria(''); return }
    setLoadingCats(true)
    supabase.from('categorias').select('categoria').eq('empresa', empresa).then(({ data }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: errIns } = await supabase.from('registro_orcamentos').insert({
      empresa,
      categoria,
      mes: mesNum,
      ano: anoNum,
  ...
  const downloadTemplate = async () => {
    const res = await fetch('/api/orcamentos/template')
    if (!res.ok) return
    const blob = await res.blob()
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = 'template_orcamentos.xlsx'; a.click()
  ...
  const handleLoteImport = async () => {
    ...
    const res = await fetch('/api/orcamentos/importar-lote', { method: 'POST', body: fd })
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">Registrar Orçamento de Compra</h2>
        ...
        {tab === 'lote' && (
          <div className="space-y-4">
            <div className="text-sm text-slate-600 bg-slate-50 border border-slate-200 rounded p-3 space-y-1">
              ...
              <p>• <strong>Empresa</strong>, <strong>Categoria</strong>: devem existir no cadastro</p>
...
export default function OrcamentoPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: vig }, { data: reg }, { data: cats }, u] = await Promise.all([
      supabase.from('registro_orcamento_analise').select('*').order('empresa').order('categoria').order('ano').order('mes'),
      supabase.from('registro_orcamentos').select('*').order('empresa').order('categoria').order('ano').order('mes'),
      supabase.from('categorias').select('empresa'),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="page-title">Orçamento de Compras</h1>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10">
                  <tr className="table-header">
                    ...
                    <th className="table-cell font-medium text-right">Orçamento Inicial (R$)</th>
                    <th className="table-cell font-medium text-right">Orçamento Vigente (R$)</th>
                  </tr>
                </thead>
                <tbody>
                  {vigFiltradas.map(r => (
                    <tr key={r.id} className="table-row">
                      ...
                      <td className="table-cell text-center">{MESES[r.mes - 1]}</td>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10">
                  <tr className="table-header">
                    ...
                    <th className="table-cell font-medium text-center">Mês</th>
                    <th className="table-cell font-medium text-center">Ano</th>
                    <th className="table-cell font-medium text-right">Valor (R$)</th>
