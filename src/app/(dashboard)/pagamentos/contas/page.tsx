...
export default function ContasPage() {
  ...
  const setStr = (field: keyof Conta) => (e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-title">Contas Pagadoras</h1>
          <p className="page-subtitle">Contas bancárias utilizadas nos pagamentos</p>
        </div>
                ...
                {contas.map(c => (
                  <tr key={c.id} className="table-row">
                    ...
                    <td className="table-cell text-xs">
                      {c.agencia ? `Ag ${c.agencia}${c.digito_agencia ? '-' + c.digito_agencia : ''} / CC ${c.conta_corrente ?? ''}${c.digito_conta ? '-' + c.digito_conta : ''}` : '-'}
                    </td>
      ...
      <Modal open={modal.open} onClose={closeModal} title={modal.data.id ? 'Editar Conta' : 'Nova Conta'} size="lg">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="input" maxLength={2} value={modal.data.estado ?? ''} onChange={e => setModal(m => ({ ...m, data: { ...m.data, estado: e.target.value.toUpperCase() } }))} />
          </div>
          <div>
