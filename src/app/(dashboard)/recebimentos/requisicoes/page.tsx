...
export default function RequisicoesReceitaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    const { error: err } = await supabase.from('requisicoes_receita').insert({
  ...
  if (!fluxoAtivo) {
    return (
      <div className="card flex items-start gap-2 p-4 bg-amber-50 border border-amber-200 text-amber-800">
        ...
        <p className="text-sm">
          Esta tela só está disponível quando o Fluxo 4 ou o Fluxo 5 está ativo. Ative em Painel Administrativo → Configurações (Recebimentos).
        </p>
      </div>
    )
  }
