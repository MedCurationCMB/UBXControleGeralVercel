...
export default function RecebimentosPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: peds }, { data: flx }, { data: orc }, { data: cats }] = await Promise.all([
      supabase
        ...
        .select('id, empresa, categoria, cliente, valor_pedido, status'),
      supabase
        .from('pedidos_solicitados_fluxo_receita')
        .select('empresa, categoria, mes, ano, valor_referente, status'),
      supabase
        .from('controle_orcamento_receita')
        .select('empresa, categoria, mes, ano, valor_orcamento, saldo'),
      supabase
  ...
  const chartValores = useMemo(() => {
    ...
    return periodos.map(periodo => {
      ...
      const saldo = Math.max(0, orcFiltrado
        .filter(o => o.mes === mesIdx && o.ano === ano)
        .reduce((s, o) => s + o.saldo, 0))

      return { periodo, Autorizado: autorizado, Aguardando: aguardando, 'Meta Restante': saldo }
    })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-title">Recebimentos — Visão Geral</h1>
          <p className="page-subtitle">Resumo e análise dos pedidos de recebimento</p>
        </div>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={380}>
            <BarChart data={chartValores} margin={{ bottom: 60 }} barGap={2}>
              ...
              <Bar dataKey="Meta Restante" fill="#94a3b8" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}

      {/* Gráfico 2 — Metas por Período */}
      {anos.length > 0 && (
        <div className="card">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Metas por Período</h2>
          <ResponsiveContainer width="100%" height={380}>
              ...
              <tbody>
                {pedidosTabela.slice(0, 100).map(p => (
                  <tr key={p.id} className="table-row">
                    ...
                    <td className="table-cell font-medium">{p.cliente}</td>
