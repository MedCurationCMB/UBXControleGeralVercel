...
const STORAGE_KEY = 'acompanhar-recebimentos-v1'
...
const ORDEM_SQL: Record<Ordem, { col: string; asc: boolean; label: string }> = {
  ...
  parte:      { col: 'cliente',        asc: true,  label: 'Cliente (A–Z)' },
}
...
export default function AcompanharRecebimentosPage() {
  ...
  const filtrosKey = chaveFiltros(filtroEmpresa, filtroCategoria, filtroCliente, filtroStatus, activeSearchId, ordem)
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        filtrosAnterior.current = chaveFiltros(s.filtroEmpresa, s.filtroCategoria, s.filtroCliente, s.filtroStatus, s.activeSearchId, ordemOk)
        ...
        setFiltroCliente(s.filtroCliente ?? '')
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify({
        view, page, searchId, activeSearchId, filtroEmpresa, filtroCategoria, filtroCliente, filtroStatus, ordem, kanbanLimit,
      }))
    } catch {}
  }, [restaurado, view, page, searchId, activeSearchId, filtroEmpresa, filtroCategoria, filtroCliente, filtroStatus, ordem, kanbanLimit])
  ...
  const load = useCallback(async () => {
    ...
    setTotal(count ?? 0)
    setLoading(false)
  }, [page, activeSearchId, filtroEmpresa, filtroCategoria, filtroCliente, filtroStatus, ordem])
  ...
  const loadKanban = useCallback(async () => {
    ...
    for (let from = 0; from < kanbanLimit; from += 1000) {
      ...
      let query = supabase
        .from('pedidos_solicitados_receita')
        .select('id, empresa, categoria, cliente, valor_pedido, status, emergencia, data_solicitacao, cancelado', { count: 'exact' })
    ...
    const respostas = await Promise.all(blocos.map(b =>
      supabase.from('controle_recebimento').select('pedido_id, valor_pagar, valor_pagamento').in('pedido_id', b)))
  ...
  }, [filtroEmpresa, filtroCategoria, filtroCliente, ordem, kanbanLimit])
