...
function ImportModal({ onClose }: { onClose: () => void }) {
  ...
  const downloadTemplate = async () => {
    const res = await fetch('/api/pedidos-receita/template-importacao')
    if (!res.ok) return
    const blob = await res.blob()
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'template_pedidos_receita.xlsx'
    a.click()
  ...
  const handleImport = async () => {
    ...
    const res = await fetch('/api/pedidos-receita/importar-lote', { method: 'POST', body: formData })
...
// ---- Main Page ----
export default function SolicitarRecebimentoPage() {
  // Step 1 form
    ...
    <div className="space-y-6">
      ...
      {step === 'form' && (
        <form onSubmit={handleInitialSubmit} className="card space-y-5">
          ...
          <div>
            <label className="label">Descrição da Venda/Contratação</label>
            <textarea
              className="input min-h-[80px] resize-y"
              placeholder="Descreva detalhes da venda ou contratação..."
              value={observacao}
          ...
          <div className="flex justify-end">
            <button type="submit" className="btn-primary px-8" disabled={saving}>
              {saving ? 'Salvando...' : 'Adicionar Fluxo de Recebimento'}
            </button>
          </div>
        </form>
      )}
      ...
      {step === 'meses' && (
        <div className="space-y-5">
          ...
          {saldos.length > 0 && (
            <div className="card">
              <h2 className="text-sm font-semibold text-slate-900 mb-3">Metas Disponíveis</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="table-header">
                      <th className="table-cell font-medium text-left">Período</th>
                      <th className="table-cell font-medium text-right">Valor Restante para atingir a Meta</th>
                    </tr>
                  </thead>
                  <tbody>
                    {saldos.map(s => (
                      <tr key={s.mesLabel} className="table-row">
                        ...
                        <td className={`table-cell text-right font-medium ${s.saldo <= 0 ? 'text-green-700' : 'text-slate-700'}`}>
                          {s.saldo <= 0 ? 'Meta Batida ✓' : fmtMoeda(s.saldo)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="card">
            ...
            <form onSubmit={handleAddMes}>
              ...
              >
                <Plus size={16} />
                {addingMes ? 'Verificando meta...' : 'Adicionar Valor'}
              </button>
            </form>
          </div>
